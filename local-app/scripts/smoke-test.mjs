...
const outFile = 'data/smoke-output.txt';
...
const tooLarge = registry.run('write_file', {
  path: 'data/too-large.txt',
  content: 'a'.repeat(1024 * 1024 + 1)
}, config.workspaceRoot);
assert.equal(tooLarge.ok, false);
assert.match(tooLarge.output, /too large/i);

const listFileAsDir = registry.run('list_files', { path: 'README.md' }, config.workspaceRoot);
assert.equal(listFileAsDir.ok, false);
assert.match(listFileAsDir.output, /not a directory|failed/i);
...
const directive = planResponse('tool:list_files path=.', session.messages);
